...
/*   Updated: 2025/04/23 17:52:47 by rmota-ma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	gettimeofday(&prog()->philo[id].cycle, NULL);
	...
	if (prog()->stop == 0)
	{
		...
		printf("%ld %d has taken a fork\n", (prog()->philo[id].current.tv_sec
			...
			+ (prog()->philo[id].current.tv_usec - prog()->start.tv_usec)
			/ 1000, id + 1);
	}
...
void	eating(int id)
{
	...
	if (prog()->philo[id].eat_counter == prog()->args[4]
		&& prog()->argc == 6 && prog()->philo[id].stop == 0)
		finish_eating(id);
...
void	*lone_philo(void *id)
{
	...
	printf("%ld %d died\n", (prog()->philo[*(int *)id].current.tv_sec
			- prog()->start.tv_sec) * 1000
		+ (prog()->philo[*(int*)id].current.tv_usec
			- prog()->start.tv_usec) / 1000, *(int*)id + 1);
