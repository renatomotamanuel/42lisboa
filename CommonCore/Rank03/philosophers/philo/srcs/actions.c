...
/*   Updated: 2025/04/23 18:02:16 by rmota-ma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	philo_sleep(int id)
{
	...
	if (prog()->stop == 0)
	{
		...
		printf("%ld %d is sleeping\n", (prog()->philo[id].current.tv_sec
				- prog()->start.tv_sec) * 1000
			+ (prog()->philo[id].current.tv_usec
				- prog()->start.tv_usec) / 1000, id + 1);
	}
...
void	death_checker(int id, int sleep_time, long diff)
{
	diff = 0;
	gettimeofday(&prog()->philo[id].current, NULL);
	if (((prog()->philo[id].current.tv_sec - prog()->philo[id].cycle.tv_sec)
			* 1000 + (prog()->philo[id].current.tv_usec
				- prog()->philo[id].cycle.tv_usec) / 1000)
		+ sleep_time >= prog()->args[1])
		grim_reaper(id, sleep_time, diff);
	else
...
void	grim_reaper(int id, int sleep_time, long diff)
{
	...
	diff = (((prog()->philo[id].current.tv_sec - prog()->philo[id].cycle.tv_sec)
				* 1000 + (prog()->philo[id].current.tv_usec
					- prog()->philo[id].cycle.tv_usec) / 1000)
			+ sleep_time) - prog()->args[1];
	pthread_mutex_lock(&prog()->death);
